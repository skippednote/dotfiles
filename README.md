...
- `~/.claude/settings.json` and `settings.local.json` — Claude Code rewrites
  both, and they carry machine-specific state. **A rebuilt machine needs these
  keys restored by hand**, because nothing here tracks them and their loss is
  silent: `hooks` (the `rtk hook claude` PreToolUse hook on Bash),
  `permissions.defaultMode`, `env` (the `PILOT_*` review flags), `model`,
  `modelSettings` and `extraKnownMarketplaces`. The original migration linked
  the repo's copy over the live file and dropped all six; RTK stopped
  rewriting commands for three days and nothing reported it.
- `~/.codex/config.toml` — regenerated by its own tool
