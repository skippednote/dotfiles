...
- Never attribute commits to Claude or any AI assistant — no `Co-Authored-By` trailers, no "Generated with Claude Code" footer. Never reference Claude or any AI assistant in commit messages or PR descriptions. Write them as if authored by me.

## Verifying work

- Verify by running the real thing to a point where a user would see it — not typechecking or tests alone.
- Use Claude-in-Chrome to validate web behavior and to log into dashboards; don't assume it works.
...
## Infra & secrets (when applicable)

- Prefer Terraform for infra changes; avoid drift (commit/push the tf). Default to dev/non-prod unless told otherwise.
- Prefer SSO / SSM / OIDC over static credentials; prefer official CLIs (gh, aws).
- Never print or commit secret values — reference locations only (SOPS, 1Password). Don't re-raise items I've parked ("rotate later").
