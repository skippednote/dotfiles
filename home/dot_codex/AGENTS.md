...
- Never attribute commits to Codex or any AI assistant — no `Co-Authored-By` trailers, no "Generated with Codex" footer. Never reference Codex or any AI assistant in commit messages or PR descriptions. Write them as if authored by me.

## Verifying work

- Verify by running the real thing to a point where a user would see it — not typechecking or tests alone.
- Use the browser to validate web behavior and to log into dashboards; don't assume it works.
